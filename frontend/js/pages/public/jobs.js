      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-start">
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-4 flex flex-wrap gap-3 text-xs font-bold text-slate-500">
            <span class="inline-flex items-center gap-1"><span class="material-symbols-outlined text-sm">location_on</span>${esc(location)}</span>
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            ${skills.length ? skills.slice(0, 4).map(skill => `<span class="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-bold text-blue-700">${esc(skill)}</span>`).join('') : '<span class="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-500">Chưa cập nhật kỹ năng</span>'}
          </div>
        </div>
